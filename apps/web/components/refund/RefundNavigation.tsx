...
import { REFUND_SECTIONS, type RefundSection } from "../../app/refund-policy/refund-policy-content";

interface RefundNavigationProps {
  sections?: readonly RefundSection[];
}

export function RefundNavigation({
  sections = REFUND_SECTIONS,
}: RefundNavigationProps) {
  const [activeId, setActiveId] = React.useState<string>(
    sections[0]?.id ?? "non-refundable-purchases"
  );
  ...
  return (
    <nav
      aria-label="Refund policy document index"
      className="
    ...
    >
      ...
      <div className="mt-8 border-t border-border pt-4 dark:border-white/[0.08]">
        <div className="flex items-center gap-2 text-[10px] text-muted-foreground dark:text-zinc-500">
          ...
          <span>{sections.length} Active Policy Sections</span>
        </div>
      </div>
    </nav>
  );
