...
import { BEFORE_YOU_PURCHASE_ITEMS } from "../../app/refund-policy/refund-policy-content";

export function BeforeYouPurchaseCard() {
  return (
    <section
    ...
    >
      <div className="mx-auto max-w-[1240px] px-5 sm:px-8 lg:px-10">
        ...
        >
          Four things to know before completing an order.
        </h2>
        ...
        <div className="mt-10 divide-y divide-border border-y border-border dark:divide-white/[0.08] dark:border-white/[0.08]">
          {BEFORE_YOU_PURCHASE_ITEMS.map((item) => (
            <div
              key={item.index}
              className="grid gap-3 py-6 sm:grid-cols-[200px_1fr] sm:gap-8 lg:grid-cols-[240px_1fr] lg:py-7"
            >
              <div>
                <div className="flex items-center gap-3">
                ...
                <span className="mt-1 block font-mono text-[9px] uppercase tracking-[0.1em] text-muted-foreground dark:text-zinc-500">
                  {item.summary}
                </span>
              </div>
        ...
        <div className="mt-6 flex items-center gap-2 text-xs text-muted-foreground dark:text-zinc-500">
          ...
          <p className="italic">
            These commercial summaries explain our core principles but do not replace
            the complete legal sections set out below.
          </p>
        </div>
      </div>
    </section>
