...
import { TERMS_SECTIONS, type TermsSection } from "../../app/terms/terms-content";

interface MobileTermsNavigationProps {
  sections?: readonly TermsSection[];
}

export function MobileTermsNavigation({
  sections = TERMS_SECTIONS,
}: MobileTermsNavigationProps) {
  const [open, setOpen] = React.useState(false);

  const scrollToSection = (id: string) => {
    setOpen(false);
    // Give time for sheet exit animation then scroll smoothly
    setTimeout(() => {
        ...
        >
          <SheetHeader className="text-left">
            ...
            <SheetDescription className="font-mono text-[7px] uppercase tracking-[0.14em] text-muted-foreground dark:text-zinc-500">
              Jump directly to any legal section
            </SheetDescription>
          </SheetHeader>

          <hr className="my-4 border-border dark:border-white/[0.08]" />

          <nav aria-label="Mobile terms sections index" className="flex flex-col gap-1">
            {sections.map((section) => (
              <button
                ...
                className="
                  ...
                  rounded-md px-3 py-2 text-left text-xs
