...
export function TermsNavigation({
  ...
  const [activeId, setActiveId] = React.useState<string>(sections[0]?.id ?? "acceptance");
  ...
  const scrollToSection = (e: React.MouseEvent<HTMLAnchorElement>, id: string) => {
    ...
    if (element) {
      const yOffset = -80; // offset for sticky 64px header
  ...
  return (
    <nav
      aria-label="Terms of service document index"
      className="
    ...
    >
      ...
      <div className="mt-6 border-t border-border pt-4 dark:border-white/[0.08]">
        <div className="flex items-center gap-2 text-[10px] text-muted-foreground dark:text-zinc-500">
          ...
          <span>{sections.length} Active Terms Sections</span>
        </div>
      </div>
    </nav>
  );
