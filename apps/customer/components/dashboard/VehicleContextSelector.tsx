...
import { Popover, PopoverContent, PopoverTrigger } from "@vaahansafe/ui";
...
export function VehicleContextSelector({
  ...
  if (!activeVehicle) {
    return (
      <div className="flex h-10 items-center gap-2 rounded-xl border border-dashed border-border px-3 font-mono text-xs text-muted-foreground">
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          type="button"
          aria-expanded={open}
          className="group flex h-10 items-center gap-2.5 rounded-xl border border-border bg-card px-3 text-left shadow-2xs transition-all hover:border-[#cc785c]/40 hover:shadow-xs focus:outline-none focus:ring-2 focus:ring-[#cc785c]/30"
        >
      ...
      <PopoverContent align="end" className="w-80 p-0 shadow-lg">
        <div className="border-b border-border p-3">
          ...
          <div className="mt-2 flex items-center gap-2 rounded-lg border border-border bg-background px-2.5 py-1.5 text-xs text-foreground focus-within:border-[#cc785c]">
            <VaahanIcon name="search" size={13} className="text-muted-foreground" />
            <input
              type="text"
          ...
          {filteredVehicles.map((v) => {
            ...
            return (
              ...
              >
                ...
                {isSelected && (
                  <span className="font-mono text-[10px] text-[#cc785c]">ACTIVE</span>
                )}
              </button>
            );
          })}
