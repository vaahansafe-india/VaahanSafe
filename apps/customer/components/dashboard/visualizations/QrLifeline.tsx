        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border pb-3">
          ...
          {qrSticker && qrSticker.status === "ACTIVE" && (
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg border border-border bg-muted/50 px-2.5 py-1 font-mono text-[11px] font-medium text-foreground hover:border-[#cc785c] hover:text-[#cc785c]"
            >
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-baseline sm:justify-between gap-1">
                      <div className="font-mono text-xs font-bold text-foreground group-hover:text-[#cc785c]">
                        {evt.title}
                      </div>
                      <time className="font-mono text-[10px] text-muted-foreground">
                        {evt.timestamp ? evt.timestamp.slice(0, 16).replace("T", " ") : "Recorded"}
                      </time>
                    </div>
                    ...
                    <div className="mt-1 flex items-center gap-2 font-mono text-[9px] text-muted-foreground uppercase">
                      <span>Actor: {evt.actorType}</span>
                      {evt.reasonCode && (
                        <>
                          <span>&bull;</span>
                          <span>Reason: {evt.reasonCode}</span>
                        </>
                      )}
                    </div>
                  </li>
