...
export interface UnavailableQrStateProps {
  publicId: string;
}

export function UnavailableQrState({ publicId }: UnavailableQrStateProps) {
  ...
  return (
    <div className="w-full p-6 rounded-2xl border border-border/80 bg-card text-center space-y-5 shadow-xs animate-in fade-in duration-300">
      <div className="mx-auto size-12 rounded-2xl bg-muted text-muted-foreground border border-border flex items-center justify-center">
      ...
      <div className="space-y-1.5">
        <Badge
          variant="outline"
          className="font-mono text-[10px] tracking-wider text-muted-foreground border-border uppercase px-2.5 py-0.5"
        >
          Retired Identity
        </Badge>
        ...
        <p className="text-xs text-muted-foreground leading-relaxed max-w-sm mx-auto">
          This QR sticker has been retired or reported unusable. Emergency routing is inactive.
        </p>
      </div>
